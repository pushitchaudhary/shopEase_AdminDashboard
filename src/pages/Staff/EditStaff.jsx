...
function EditStaff() {
    ...
    const {singleStaffDetail, alertData, staffStatus, error} = useSelector((state)=>state.staffData)

    // Fetch Single Staff Value
    useEffect(()=>{
        dispatch(fetchSingleStaffDetails(staffId))
    },[])

    // Storing Value in field
    useEffect(()=>{
        if(singleStaffDetail){
            setName(singleStaffDetail.name)
            setEmail(singleStaffDetail.email)
            setPhone(singleStaffDetail.phoneNumber)
            setDateOfBirth(singleStaffDetail.dateOfBirth)
            setGender(singleStaffDetail.gender)
            setAddress(singleStaffDetail.address)
            setStatus(singleStaffDetail.status)
        }
    },[singleStaffDetail])
        ...
        <div class="bg-gray-100 p-8 rounded-md w-full">
            <div class=" flex items-center justify-between pb-6">
                <div className='flex'>
                    ...
                    <h2 class="text-blue-800 text-xl font-semibold uppercase">Edit Staff</h2>
                </div>
            </div>
            <div className='mt-4'>
